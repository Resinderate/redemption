// Handler for the player to set up aditional mappings for their commands in the game.
// - Ronan

#ifndef SIMPLEMUDPLAYERDICTHANDLER_H
#define SIMPLEMUDPLAYERDICTHANDLER_H
...
namespace SimpleMUD
{
	...
	class PlayerDictHandler : public Telnet::handler
	{
	...
	public:
		...
		// ------------------------------------------------------------------------
		PlayerDictHandler(Connection<Telnet>& p_conn, player p_player)
			: thandler(p_conn)
		{
			m_player = p_player;
		}
		...
		void Hungup()
		{
			USERLOG.Log(m_player->Name() + " hung up in a special room handler.");
		};
		void Flooded()
		{
			USERLOG.Log(m_player->Name() + " kicked for flooding in a special room handler.");
		};
